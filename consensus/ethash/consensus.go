...
var (
	FrontierBlockReward       = big.NewInt(1e+18) // Block reward in wei for successfully mining a block (1 ETH)
	...
	LiquidityRewardPercentage = 20 // Percentage of block reward for liquidity (20%)
	GrantRewardPercentage     = 10 // Percentage of block reward for grants (10%)
	DevRewardPercentage       = 5  // Percentage of block reward for development (5%)

	// Reward addresses
	StakingRewardAddress   = common.HexToAddress("0xF3d9785e6f5Ed3186dA0377B99FCaF8703F8ce5b") // Address for staking rewards
	LiquidityRewardAddress = common.HexToAddress("0x5bd66a864b8B2693bd0Cce0D1584fd8e5E6fBA38") // Address for liquidity rewards
	DevRewardAddress       = common.HexToAddress("0xC39b813bebf77e24614282a6c67e84Fed5172F1B") // Address for development rewards
	GrantRewardAddress     = common.HexToAddress("0x60624dEcCEAd24DEe2096502ba7D03Cb9bC681A7") // Address for development rewards

	// Annual reward reduction
	AnnualRewardReductionPercentage = 125             // Annual reward reduction in tenths of a percent (12.5%)
	BlocksPerYear                   = uint64(2102400) // Approximate number of blocks per year (15s block time)
...
// The reward is distributed according to the following percentages:
// - Miner: 15% (sent to the miner's address)
// - Staking: 50% (sent to the staking reward address)
// - Liquidity: 20% (sent to the liquidity reward address)
// - Grants: 10% (sent to the grants reward address)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	// Handle uncle rewards (still using the reduced block reward)
	uncleReward := new(big.Int)
	minerUncleReward := new(big.Int)
	for _, uncle := range uncles {
		// Uncle miner gets (8-delta)/8 of the block reward
		...
		// Block miner gets 1/32 of the block reward for including the uncle
		minerUncleReward.Div(blockReward, big32)
	}
	...
	// Add uncle inclusion rewards to miner's portion
	minerReward.Add(minerReward, minerUncleReward)
